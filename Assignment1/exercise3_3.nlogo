turtles-own [
  ...
  birthday        ;; the number of ticks when a turtle is born GW
  lifespan        ;; how many ticks the turtle will surive (given enough food) GW
]
...
to turtle-setup ;; turtle procedure
  make-default-turtle 0; current day is 0
  run visualization
end

to make-default-turtle [current-day]
  set color red
  set shape "circle"
  set lifespan random-in-range 60 100
  set birthday current-day
  (ifelse
...
to go
  ...
  ask turtles [
    ...
    [
      if ((ticks - birthday) >= lifespan
           or sugar <= 0
      )[
        ; die ask observer [ create-turtles 1 ]; unfortunately does not work
        make-default-turtle ticks; kill the old turtle and make a new one
      ]
    ]
    [
